/**
 * @file testMPSRandStability.cu
 * @brief Stress-test MPS rand (uniform) generation for non-finite values and range violations.
 */
...
int compute() {
    std::cout << "testMPSRandStability requires APPLE_SILICON. Skipping.\n";
    return 0;
}
#else
...
int compute() {
    ...
    std::cout << "[INFO] rand stress: shape=" << rows << "x" << cols
              << " iters=" << iters << std::endl;

    for (int it = 1; it <= iters; ++it) {
        auto x_m = Matrix<MPSfloat>::rand(rows, cols);
        auto x_h = x_m.to_host();

        size_t bad_r = 0, bad_c = 0;
        float bad_v = 0.0f;
        if (find_non_finite(x_h, bad_r, bad_c, bad_v)) {
            std::cout << "[FAIL] non-finite rand at iter=" << it
                      << " row=" << bad_r
                      << " col=" << bad_c
        ...
        for (size_t c = 0; c < x_h.num_col(); ++c) {
            for (size_t r = 0; r < x_h.num_row(); ++r) {
                const float v = x_h.elem(r, c);
                if (v < 0.0f || v >= 1.0f) {
                    std::cout << "[FAIL] out-of-range rand at iter=" << it
                              << " row=" << r
                              << " col=" << c
                              << " value=" << v << std::endl;
                    return 1;
    ...
    std::cout << "[PASS] testMPSRandStability" << std::endl;
    return 0;
}
#endif
