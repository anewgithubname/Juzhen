/**
 * @file demo_cnn_mnist_cudnn.cu
 * @brief MNIST classification using a CNN (CUDA + cuDNN implementation) with live FTXUI display
 */
...
#if !defined(CUDA)
int compute() {
    cout << "demo_cnn_mnist_cudnn requires NVIDIA_CUDA=ON." << endl;
    return 0;
}
#elif !defined(CUDNN_AVAILABLE)
int compute() {
    cout << "demo_cnn_mnist_cudnn requires cuDNN (CUDNN_AVAILABLE)." << endl;
    return 0;
}
#else
...
int compute() {
    ...
    const std::string loss_path = env_str("CNN_MNIST_LOSS_PATH",
        PROJECT_DIR + std::string("/res/cnn_mnist_cudnn_loss.csv"));
    ...
    auto renderer = Renderer([&] {
        ...
        auto header = text(" CNN MNIST  [cuda+cudnn]   epochs=" + std::to_string(n_epochs)
    ...
    std::thread train_thread([&] {
        ...
        // GPU vis batch (full batchsize, normalized)
        auto vis_input = Matrix<CUDAfloat>(vis_X_cpu / 255.0f);
        {
            std::lock_guard<std::mutex> lk(mtx);
        ...
        ReluLayer<CUDAfloat>   L1(fc_hidden, conv_out_dim, batchsize);
        LinearLayer<CUDAfloat> L2(k,         fc_hidden,    batchsize);
        list<Layer<CUDAfloat>*> trainnn = { &L2, &L1, &conv3, &conv2, &conv1 };

        std::mt19937 init_rng(seed);
        for (auto* layer : trainnn) {
            layer->W() = Matrix<CUDAfloat>(make_host_random_matrix(layer->W().num_row(), layer->W().num_col(), init_rng, 0.01f));
            layer->b() = Matrix<CUDAfloat>(make_host_random_matrix(layer->b().num_row(), layer->b().num_col(), init_rng, 0.01f));
        }
        ...
        for (int epoch = 0; epoch < n_epochs && !stop_early; ++epoch) {
            for (int step = 0; step < steps_per_epoch && !stop_early; ++step) {
                ...
                auto X_i = Matrix<CUDAfloat>(X_i_cpu / 255.0f);
                auto Y_i = Matrix<CUDAfloat>(one_hot(y_i_idx, k));

                forward(trainnn, X_i);
                LogisticLayer<CUDAfloat> loss_layer(batchsize, std::move(Y_i));
            ...
            // Run inference on fixed vis batch to update sample predictions
                ...
                if (do_vis) {
                    list<Layer<CUDAfloat>*> infnn = { &L2, &L1, &conv3, &conv2, &conv1 };
                    auto out_gpu = forward(infnn, vis_input); // (k, batchsize) on GPU
                    auto out = out_gpu.to_host();
                    preds.resize(n_vis);
                    for (int i = 0; i < n_vis; ++i) {
            ...
            for (size_t b = 0; b < n_test_batches; ++b) {
                ...
                auto XT_b = Matrix<CUDAfloat>(XT_b_cpu / 255.0f);
                auto YT_b = Matrix<CUDAfloat>(one_hot(yT_b_idx, k));
                ZeroOneLayer<CUDAfloat> eval_loss(batchsize, std::move(YT_b));
                list<Layer<CUDAfloat>*> evalnn = { &eval_loss, &L2, &L1, &conv3, &conv2, &conv1 };
                total_err += forward(evalnn, XT_b).to_host().elem(0, 0);
            }
            std::lock_guard<std::mutex> lk(mtx);
