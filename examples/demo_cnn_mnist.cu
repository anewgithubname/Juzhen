/**
 * @file demo_cnn_mnist.cu
 * @brief MNIST classification using a CNN with live FTXUI display.
 *        Supports CPU, CUDA+cuDNN, ROCm/HIP, and MPS backends.
 */
...
// ── Backend selection ──────────────────────────────────────────────────
    ...
    using D = CUDAfloat;
    static constexpr const char* BACKEND_LABEL = "cuda+cudnn";
    static constexpr const char* LOSS_SUFFIX   = "cudnn";
#elif defined(ROCM_HIP)
    using D = ROCMfloat;
    static constexpr const char* BACKEND_LABEL = "rocm/hip";
    static constexpr const char* LOSS_SUFFIX   = "rocm";
#elif defined(APPLE_SILICON)
    using D = MPSfloat;
    static constexpr const char* BACKEND_LABEL = "mps";
    static constexpr const char* LOSS_SUFFIX   = "mps";
#else
    using D = float;
    static constexpr const char* BACKEND_LABEL = "cpu";
    static constexpr const char* LOSS_SUFFIX   = "cpu";
#endif

// ── Helpers ────────────────────────────────────────────────────────────

template<typename T>
Matrix<float> as_host(const Matrix<T>& m) {
...
static int env_int(const char* key, int default_v) {
    ...
    return v ? std::atoi(v) : default_v;
}

static std::string env_str(const char* key, const std::string& default_v) {
    const char* v = std::getenv(key);
    return v ? std::string(v) : default_v;
}

static Matrix<float> make_host_random_matrix(size_t rows, size_t cols,
                                              std::mt19937& rng, float stddev) {
...
int compute() {
    ...
    const int vis_every = 100;
    const std::string loss_path = env_str("CNN_MNIST_LOSS_PATH",
        PROJECT_DIR + std::string("/res/cnn_mnist_") + LOSS_SUFFIX + "_loss.csv");
    ...
    auto renderer = Renderer([&] {
        ...
        auto header = text(std::string(" CNN MNIST  [") + BACKEND_LABEL + "]   epochs="
                           + std::to_string(n_epochs) + "   seed=" + std::to_string(seed)) | bold;
    ...
    std::thread train_thread([&] {
        ...
        auto vis_input = Matrix<D>(vis_X_cpu / 255.0f);
        {
            std::lock_guard<std::mutex> lk(mtx);
        ...
        ReluLayer<D>   L1(fc_hidden, conv_out_dim, batchsize);
        LinearLayer<D> L2(k,         fc_hidden,    batchsize);
        list<Layer<D>*> trainnn = { &L2, &L1, &conv3, &conv2, &conv1 };

        std::mt19937 init_rng(seed);
        for (auto* layer : trainnn) {
            layer->W() = Matrix<D>(make_host_random_matrix(
                layer->W().num_row(), layer->W().num_col(), init_rng, 0.01f));
            layer->b() = Matrix<D>(make_host_random_matrix(
                layer->b().num_row(), layer->b().num_col(), init_rng, 0.01f));
        }
        ...
        for (int epoch = 0; epoch < n_epochs && !stop_early; ++epoch) {
            for (int step = 0; step < steps_per_epoch && !stop_early; ++step) {
                ...
                auto X_i = Matrix<D>(X_i_cpu / 255.0f);
                auto Y_i = Matrix<D>(one_hot(y_i_idx, k));

                forward(trainnn, X_i);
                LogisticLayer<D> loss_layer(batchsize, std::move(Y_i));
                ...
                if (do_vis) {
                    list<Layer<D>*> infnn = { &L2, &L1, &conv3, &conv2, &conv1 };
                    auto out = as_host(forward(infnn, vis_input));
                    preds.resize(n_vis);
                    for (int i = 0; i < n_vis; ++i) {
            ...
            for (size_t b = 0; b < n_test_batches; ++b) {
                ...
                auto XT_b = Matrix<D>(XT_b_cpu / 255.0f);
                auto YT_b = Matrix<D>(one_hot(yT_b_idx, k));
                ZeroOneLayer<D> eval_loss(batchsize, std::move(YT_b));
                list<Layer<D>*> evalnn = { &eval_loss, &L2, &L1, &conv3, &conv2, &conv1 };
                total_err += as_host(forward(evalnn, XT_b)).elem(0, 0);
            }
            std::lock_guard<std::mutex> lk(mtx);
