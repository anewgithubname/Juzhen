/**
 * @file demo_cnn_mnist_mps.cu
 * @brief MNIST classification using a CNN (MPS implementation) with live FTXUI display
 */
...
int compute() {
    cout << "demo_cnn_mnist_mps requires APPLE_SILICON=ON." << endl;
    return 0;
}
#else
...
int compute() {
    ...
    const std::string loss_path = env_str("CNN_MNIST_LOSS_PATH",
        PROJECT_DIR + std::string("/res/cnn_mnist_mps_loss.csv"));
    ...
    auto renderer = Renderer([&] {
        ...
        auto header = text(" CNN MNIST  [mps]   epochs=" + std::to_string(n_epochs)
    ...
    std::thread train_thread([&] {
        ...
        // MPS vis batch (full batchsize, normalized)
        auto vis_input = Matrix<MPSfloat>(vis_X_cpu / 255.0f);
        {
            std::lock_guard<std::mutex> lk(mtx);
        ...
        ReluLayer<MPSfloat>   L1(fc_hidden, conv_out_dim, batchsize);
        LinearLayer<MPSfloat> L2(k,         fc_hidden,    batchsize);
        list<Layer<MPSfloat>*> trainnn = { &L2, &L1, &conv3, &conv2, &conv1 };

        std::mt19937 init_rng(seed);
        for (auto* layer : trainnn) {
            layer->W() = Matrix<MPSfloat>(make_host_random_matrix(layer->W().num_row(), layer->W().num_col(), init_rng, 0.01f));
            layer->b() = Matrix<MPSfloat>(make_host_random_matrix(layer->b().num_row(), layer->b().num_col(), init_rng, 0.01f));
        }
        ...
        for (int epoch = 0; epoch < n_epochs && !stop_early; ++epoch) {
            for (int step = 0; step < steps_per_epoch && !stop_early; ++step) {
                ...
                auto X_i = Matrix<MPSfloat>(X_i_cpu / 255.0f);
                auto Y_i = Matrix<MPSfloat>(one_hot(y_i_idx, k));

                forward(trainnn, X_i);
                LogisticLayer<MPSfloat> loss_layer(batchsize, std::move(Y_i));
            ...
            // Run inference on fixed vis batch to update sample predictions
                ...
                if (do_vis) {
                    list<Layer<MPSfloat>*> infnn = { &L2, &L1, &conv3, &conv2, &conv1 };
                    auto out = forward(infnn, vis_input).to_host();
                    preds.resize(n_vis);
                    for (int i = 0; i < n_vis; ++i) {
            ...
            for (size_t b = 0; b < n_test_batches; ++b) {
                ...
                auto XT_b = Matrix<MPSfloat>(XT_b_cpu / 255.0f);
                auto YT_b = Matrix<MPSfloat>(one_hot(yT_b_idx, k));
                ZeroOneLayer<MPSfloat> eval_loss(batchsize, std::move(YT_b));
                list<Layer<MPSfloat>*> evalnn = { &eval_loss, &L2, &L1, &conv3, &conv2, &conv1 };
